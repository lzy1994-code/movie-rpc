...
//            File outputFile = new File(filePath);
     ...
     * @param toEmail
     */
    public static void createQRcodeToEmail(String codeContent, String title, String content, String toEmail) {
        try {
            int width = 300;
            int height = 300;
            Hashtable hints = new Hashtable();
            hints.put(EncodeHintType.CHARACTER_SET, "utf-8");
            BitMatrix bitMatrix = new MultiFormatWriter().encode(codeContent, BarcodeFormat.QR_CODE, width, height, hints);
            BufferedImage bufferedImage = MatrixToImageWriter.toBufferedImage(bitMatrix);
            InputStream inputStream = new ByteArrayInputStream(imageToByte(bufferedImage, "png"));
            SendEmailUtil sendEmailUtil = SendEmailUtil.init();
            sendEmailUtil.sendEmail(toEmail, title, content, inputStream);
        } catch (WriterException e) {
            e.printStackTrace();
        } catch (IOException e) {
            e.printStackTrace();
